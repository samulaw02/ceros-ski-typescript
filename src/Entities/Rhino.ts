...
export class Rhino extends AnimatedEntity {
    /**
     ...
     */
    state: string = STATES.STATE_RUNNING
     ...
     */
    update(gameTime: number, target: AnimatedEntity) {
        if (this.isRunning()) {
            this.move(target);
     ...
     */
    checkIfCaughtTarget(target: AnimatedEntity) {
     ...
     */
    caughtTarget(target: AnimatedEntity) {
        target.die();
